...
func CreateChannelCostVersion(c *gin.Context) {
	...
	// discount 模式冻结服务端当前汇率，与迁移回填（seedLoadExchangeRate）使用同一来源。
	// InitOptionMap 在启动时已将 options 表的值同步到 operation_setting.USDExchangeRate，
	// 此处直接读包级变量即可，无需再查库。
...
func DeleteChannelCostVersion(c *gin.Context) {
	...
	// 先读出版本以获取 channel_id，供后续最后版本保护检查使用。
	...
	// 若此版本是该渠道仅存的最后一条，拒绝删除以保护历史定价基准。
	existing, err := model.GetChannelCostVersions(v.ChannelId)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	if len(existing) <= 1 {
		c.JSON(http.StatusOK, gin.H{"success": false,
			"message": "cannot delete the last version of a channel; add a replacement version first"})
		return
	}
